...
*/
   ...
   public ListInterface<T> getAllLessThan(T entry) {
	...
	public int removeEvery(T element) {
		...
		// YOUR HOMEWORK CODE HERE.REMOVES ALL OCCURENCES OF AN ENTRY AND RETURNS NUMBER OF ELEMENTS REMOVED
		
		// LOGIC => FOR LOOP RUNS THROUGH ALIST AND CHECK IF ARRAY ELEMENT MATCHES T ELEMENT, 
		// IF SO IT CLEARS THAT ELEMENT AND ADDS IT TO A VARIABLE COUNT_FREQUENCY
		// RETURNS COUNT_FREQUENCY
   ...
   //TWO LISTS ARE THE SAME IF THEY ARE IN THE SAME ORDER
	//LOGIC => RUN A FOR LOOP IN LIST, WHERE IF OBJECT ARRAY INDEX EQUALS TO LIST INDEX, RETURN TRUE
	@Override
	public boolean equals(Object obj) {
		if (obj instanceof AList<?>) {
			AList<T> otherListParam = (AList<T>) obj;

			AList<T> otherList = new AList<T>(otherListParam.numberOfEntries);
			for (int i = 1; i <= otherListParam.numberOfEntries; i++) {
				otherList.add(otherListParam.list[i]);
			}
	...
	public int compareTo(AList<T> otherList) {
		...
		// A = B , 0
			...
			for (int i = 1; i <= numberOfEntries; i++) {      // UN-MATCHED ELEMENT COMPARISION
				if (list[i].compareTo(otherList.list[i])> 0) {
					return 1;
					
			}else if(list[i].compareTo(otherList.list[i])< 0)
				return -1;
